...
import { ShieldAlert, ShieldCheck, Copy, Trash2, RefreshCw, AlertCircle, Check } from 'lucide-react';
import { encryptDES, decryptDES, generateSecureRandomHex } from '../utils/cryptoHelpers';
...
export default function DesView({ onAddLog, onUpdateStats, onSetStatus }: DesViewProps) {
  ...
  const handleGenerateKey = () => {
    ...
    try {
      ...
      const secureKey = generateSecureRandomHex(8); // 8 bytes = 64-bit key
      setKey(secureKey);
      ...
      onAddLog({
        ...
        status: 'Success',
        detail: 'Generated random 64-bit DES key',
        executionTime: duration
      });
      onUpdateStats('keygen');
      onSetStatus('Generated secure DES key successfully', 'success');
    } catch (err: any) {
  ...
  const handleEncrypt = () => {
    ...
    if (!inputText) {
      ...
      onSetStatus('DES Encryption error: empty input', 'error');
      return;
    }
    if (!key) {
      setError('Key is required for encryption.');
      onSetStatus('DES Encryption error: missing key', 'error');
      return;
    }

    const t0 = performance.now();
    try {
      onSetStatus('Encrypting plain text using DES-CBC...', 'info');
      const cipher = encryptDES(inputText, key);
      ...
      onAddLog({
        ...
        status: 'Success',
        detail: `Encrypted plain text (${inputText.length} chars) via legacy DES`,
        executionTime: duration
      });
      onUpdateStats('encrypt');
      onSetStatus('DES Encryption completed', 'success');
    } catch (err: any) {
      ...
      onAddLog({
        ...
        operation: 'Encryption',
        status: 'Error',
        detail: err.message || 'DES Encryption failed'
      });
      onSetStatus('DES Encryption failed', 'error');
    }
  };
  ...
  const handleDecrypt = () => {
    ...
    if (!inputText) {
      ...
      onSetStatus('DES Decryption error: empty input', 'error');
      return;
    }
    if (!key) {
      setError('Key is required for decryption.');
      onSetStatus('DES Decryption error: missing key', 'error');
      return;
    }

    const t0 = performance.now();
    try {
      onSetStatus('Decrypting cipher text using DES-CBC...', 'info');
      const decrypted = decryptDES(inputText, key);
      ...
      onAddLog({
        ...
        status: 'Success',
        detail: `Decrypted cipher text (${inputText.length} chars) via legacy DES`,
        executionTime: duration
      });
      onUpdateStats('decrypt');
      onSetStatus('Cipher text decrypted using DES successfully', 'success');
    } catch (err: any) {
      setError(err.message || 'Decryption failed. Please check 8-character key integrity.');
      onAddLog({
        algorithm: 'DES-CBC',
        operation: 'Decryption',
        status: 'Error',
        detail: err.message || 'DES Decryption failed'
      });
      onSetStatus('DES Decryption failed: corrupted parameters', 'error');
    }
  };
  ...
  const handleCopyKey = () => {
    ...
    onSetStatus('Key copied to clipboard', 'success');
  };
  ...
  const handleExport = (format: 'txt' | 'json' | 'csv') => {
    ...
    link.href = url;
    link.download = `des-output-${Date.now()}.${ext}`;
    link.click();
          ...
          <div className="bg-slate-900/40 border border-slate-900 rounded-2xl p-5 space-y-4 backdrop-blur-md shadow-md relative overflow-hidden">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-mono text-slate-400 flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500">8-CHARS OR HEX</span>
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="Enter or generate 8-byte key"
                  className="w-full bg-slate-950/70 border border-slate-800/80 rounded-xl pl-3 pr-10 py-2.5 text-xs font-mono focus:outline-none focus:border-teal-500/50 text-slate-200 placeholder-slate-600 transition-colors"
                />
                {key && (
                  <button
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-slate-950/80 border border-teal-500/30 text-teal-400 hover:bg-teal-950/30 font-medium text-xs font-mono flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm hover:shadow-[0_0_15px_rgba(20,184,166,0.15)]"
            >
          ...
          <div className="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 backdrop-blur-md shadow-md space-y-4 relative overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={5}
                className="w-full bg-slate-950/70 border border-slate-800/80 rounded-xl p-4 text-xs font-mono focus:outline-none focus:border-teal-500/50 text-slate-100 placeholder-slate-700 transition-all resize-y"
              />
            </div>
